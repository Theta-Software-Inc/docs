...
2. `getPropertyRecords`  
   Verify parcel attributes like lot size, year built, and ownership history.
3. `getPropertyById`  
   Pull detailed specs and confirm the target property’s key attributes.
4. `getValueEstimate`  
   Estimate market value using comparable sales.
5. `getRentEstimate`  
   Estimate achievable rent from comparable rentals.
6. `analyzeProperty`  
...
8. `searchSources` / `getLaborRates` / `getEquipmentRentalRates`  
   Build hard/soft cost assumptions from materials, labor, and equipment data.
9. `readDocument`  
